 ...
 */
public class LittleHeatMapActivity extends Activity implements
    ...
    private TileOverlay tileOverlayout;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        init();
        initDataAndHeatMap();
        map.moveCamera(CameraUpdateFactory.newLatLngZoom(new LatLng(34.904980,108.40955),4));

    }
    public  int[] ALT_HEATMAP_GRADIENT_COLORS = {
//            Color.argb(1, 0, 255, 255),
...
//            Color.rgb(255, 0, 0)
    ...
    public   float[] ALT_HEATMAP_GRADIENT_START_POINTS = {0.00f,
//            1.0f,1.0f};
            0.10f, 0.20f, 0.60f, 1.0f };

    public   Gradient ALT_HEATMAP_GRADIENT = new Gradient(
            ALT_HEATMAP_GRADIENT_COLORS, ALT_HEATMAP_GRADIENT_START_POINTS);

    private void initDataAndHeatMap() {
        // 第一步： 生成热力点坐标列表
        latlngs = new LatLng[20];
        double x = 34.904980;
        double y = 108.40955;
        ...
        for (int i = 0; i < 20; i++) {
            double x_ = 0;
            double y_ = 0;
//            x_ = Math.random() * 0.4 - 0.25;
//            y_ = Math.random() * 0.4 - 0.25;
            x_ = Math.random() * 4 - 0.25;
            y_ = Math.random() * 4 - 0.25;
            latlngs[i] = new LatLng(x + x_, y + y_);
        }

        // 第二步： 构建热力图 TileProvider
        HeatmapTileProvider.Builder builder = new HeatmapTileProvider.Builder();
//        builder.data(Arrays.asList(latlngs)).radius(60).transparency(1); // 设置热力图绘制的数据
//        builder.data(Arrays.asList(latlngs)).transparency(30); // 设置热力图绘制的数据
        builder.data(Arrays.asList(latlngs)).transparency(1).gradient(ALT_HEATMAP_GRADIENT); // 设置热力图绘制的数据
//        builder.data(Arrays.asList(latlngs)).transparency(0); // 设置热力图绘制的数据
//                .gradient(ALT_HEATMAP_GRADIENT); // 设置热力图渐变，有默认值 DEFAULT_GRADIENT，可不设置该接口
        ...
        // 第四步： 添加热力图
        tileOverlayout = map.addTileOverlay(tileOverlayOptions);
    }
    ...
    private void init() {
        ...
        //错误的case
        btnClear = (Button) findViewById(R.id.btn_clear_false);
    ...
    public void onClick(View view) {
        switch (view.getId()) {
            case R.id.btn_clear_false:
                tileOverlayout.remove();
                int[] ALT_HEA = {Color.argb(25, 0, 255, 255), Color.argb(255 / 3 * 2, 0, 255, 0), Color.rgb(125, 191, 0), Color.rgb(185, 71, 0), Color.rgb(255, 0, 0)};
                // 第二步： 构建热力图 TileProvider
                HeatmapTileProvider.Builder builder = new HeatmapTileProvider.Builder();
                builder.data(Arrays.asList(latlngs)).transparency(1).gradient(new Gradient(ALT_HEA,ALT_HEATMAP_GRADIENT_START_POINTS));
                // Gradient 的设置可见参考手册
                // 构造热力图对象
                heatmapTileProvider = builder.build();
                // 第三步： 构建热力图参数对象
